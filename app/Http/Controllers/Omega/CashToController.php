...
class CashToController extends Controller
{
    public function index()
    {
        if (dateOpen()) {
            if (cashOpen()) {
                ...
                return view('omega.pages.cash_to', [
                    'cash' => $cash,
                    'accounts' => $accounts,
                    'operas' => $operas,
                    'moneys' => $moneys
    ...
    public function store()
    {
        ...
        try {
            ...
            DB::commit();
            return Redirect::route('omega')->with('success', trans('alertSuccess.cashtobank'));
        } catch (\Exception $ex) {
            DB::rollBack();
            return Redirect::back()->with('success', trans('alertDanger.cashtobank'));
        }
    }
}
