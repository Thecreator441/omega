    ...
    public function index()
    {
        if (dateOpen()) {
            if (cashOpen()) {
                ...
                $maincash = Cash::getMainCash();
                $moneys = Money::getMoneys();

                return view('omega.pages.emission', [
                    'cash' => $cash,
                    'maincash' => $maincash,
                    'moneys' => $moneys
                ]);
...
//        dd(Request::all());
        ...
        $amount = trimOver(Request::input('totbil'), ' ');
        ...
        try {
            ...
            $cash = Cash::getEmpCashOpen();
            $cashto = Cash::getMainCash();
            $accdate = AccDate::getOpenAccDate();
            $opera = Operation::getByCode(7);
            $opera2 = Operation::getByCode(5);
...
//            }
            ...
            DB::commit();
            return Redirect::route('omega')->with('success', trans('alertSuccess.emisfund'));
        } catch (\Exception $ex) {
            DB::rollBack();
            return Redirect::back()->with('danger', trans('alertDanger.emisfund'));
        }
    }
}
