...
@section('content')
    <div class="box box-info" id="form" style="display: none;">
        <div class="box-header with-border">
            <h3 class="box-title text-bold"> @lang('label.newcoll')</h3>
        ...
        <div class="box-body">
            <form action="{{ route('collector/store') }}" method="post" role="form" id="instForm" enctype="multipart/form-data" class="needs-validation">
                {{ csrf_field() }}
        ...
        <div class="box-header with-border">
            ...
            <div class="box-tools pull-right">
                <button type="button" class="btn btn-success bg-green btn-sm pull-right" id="newInst">
                    <i class="fa fa-plus"></i>&nbsp;@lang('label.newcoll')
                </button>
            </div>
        </div>
        <div class="box-body">
                ...
                @foreach($collectors as $collector)
                    <tr>
                        ...
                        <td class="text-center">
                            <button type="button" class="btn btn-info bg-aqua btn-sm fa fa-edit" onclick="edit('{{$collector->idcoll}}')"></button>
                            <button type="button" class="btn bg-red btn-sm delete fa fa-trash" onclick="remove('{{$collector->idcoll}}')"></button>
                        </td>
                    </tr>
                @endforeach
                </tbody>
            </table>
            <form action="{{route('collector/delete')}}" method="post" role="form" id="delForm" style="display: none">
                {{ csrf_field() }}
...
@section('script')
    <script>
        ...
        function edit(idcollector) {
            $.ajax({
                url: "{{ url('getCollector') }}",
                method: 'get',
                data: {
                    id: idcollector
                },
                success: function (collector) {
                    $('#title').text('@lang('label.edit') ' + collector.name + ' ' + collector.surname);

                    $('#idinstitute').val(collector.idcoll);
                    $('#name').val(collector.name);
                    $('#surname').val(collector.surname);
                    $('#status').val(collector.status).trigger('change');
                    $('#gender').val(collector.gender).trigger('change');
                    $('#date').val(collector.dob);
                    $('#pob').val(collector.pob);
                    $('#post').val(collector.post);
                    $('#phone1').val(collector.phone1);
                    $('#phone2').val(collector.phone2);
                    $('#email').val(collector.email);
                    $('#nic_type').val(collector.nic_type).trigger('change');
                    $('#nic').val(collector.nic);
                    $('#idate').val(collector.issuedate);
                    $('#issueplace').val(collector.issueplace);
                    $('#country').val(collector.country).trigger('change');
                    $('#region').val(collector.region).trigger('change');
                    $('#regorigin').val(collector.regorigin).trigger('change');
                    $('#division').val(collector.division).trigger('change');
                    $('#subdiv').val(collector.subdiv).trigger('change');
                    $('#town').val(collector.town).trigger('change');
                    $('#address').val(collector.address);
                    $('#quarter').val(collector.quarter);
        ...
        function remove(idinstitute) {
            swal({
                icon: 'warning',
                title: '@lang('sidebar.collector')',
                text: '@lang('confirm.coldel_text')',
        ...
        function submitForm() {
            let text = '@lang('confirm.collsave_text')';
            if ($('#idinstitute').val() !== '') {
                text = '@lang('confirm.colledit_text')';
            }

            mySwal('@lang('sidebar.collector')', text, '@lang('confirm.no')', '@lang('confirm.yes')', '#instForm');
        }
