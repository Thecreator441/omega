...
@section('content')
    <div class="box box-info" id="newForm" style="display: none;">
        <div class="box-header with-border">
            <h3 class="box-title text-bold" id="title">@lang('label.new_menu_level_2')</h3>
        ...
        <div class="box-body">
            <form action="{{ route('menu_level_2/store') }}" method="post" role="form" id="main_menuForm" class="needs-validation">
                {{ csrf_field() }}
                ...
                <div id="fillform">
                    ...
                    <div class="row">
                        <div class="col-md-4 col-xs-12">
                            <div class="form-group has-error">
                                ...
                                <div class="col-md-9 col-xs-7">
                                    <select class="form-control select2" id="menu_level_1" name="menu_level_1" required>
                                        <option value=""></option>
                        ...
                        @if($emp->lang == 'fr')
                            <div class="col-md-4 col-xs-12">
                                <div class="form-group has-error">
                                    <label for="labelfr" class="col-md-3 col-xs-5 control-label">@lang('label.menu_level_2_fr')<span class="text-red text-bold">*</span></label>
                                    <div class="col-md-9 col-xs-7">
                                        <input type="text" class="form-control" name="labelfr" id="labelfr" required>
                                        <div class="help-block">@lang('placeholder.menu_level_2_fr')</div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-4 col-xs-12">
                                <div class="form-group has-error">
                                    <label for="labeleng" class="col-md-3 col-xs-5 control-label">@lang('label.menu_level_2_eng')<span class="text-red text-bold">*</span></label>
                                    <div class="col-md-9 col-xs-7">
                                        <input type="text" class="form-control" name="labeleng" id="labeleng" required>
                                        <div class="help-block">@lang('placeholder.menu_level_2_eng')</div>
                                    </div>
                                </div>
                            </div>
                        @else
        ...
        <div class="box-header with-border">
            ...
            <div class="box-tools pull-right">
                <button type="button" class="btn btn-success bg-green btn-sm pull-right" id="insertForm">
                    <i class="oin ion-plus"></i>&nbsp;@lang('label.new_menu_level_2')
                </button>
            </div>
        </div>
        <div class="box-body">
                    ...
                    <div class="table-responsive">
                        <table id="admin-data-table" class="table table-bordered table-striped table-hover table-responsive-xl">
                            ...
                            @foreach($main_menus_2 as $main_menu_2)
                                <tr>
                                    <td class="text-center">{{$main_menu_2->level}}</i></td>
                                    <td><span><i class="{{$main_menu_2->view_icon}}"></i></span>&nbsp;&nbsp; @if($emp->lang == 'fr') {{ $main_menu_2->labelfr }} @else {{ $main_menu_2->labeleng }} @endif</td>
                                    <td><span><i class="{{$main_menu_2->main_menu_1_view_icon}}"></i></span>&nbsp;&nbsp; {{$main_menu_2->main_menu_1}}</td>
                                    <td class="text-center">
                                        <button type="button" class="btn btn-info bg-aqua btn-sm fa fa-edit" onclick="edit('{{$main_menu_2->idmenus_2}}')"></button>
                                        <button type="button" class="btn bg-red btn-sm delete fa fa-trash" onclick="remove('{{$main_menu_2->idmenus_2}}')"></button>
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
                        <form action="{{route('menu_level_2/delete')}}" method="post" role="form" id="delForm" style="display: none">
                            {{ csrf_field() }}
    ...
    <script>
        ...
        function edit(idmain_menu) {
            $.ajax({
                ...
                success: function (main_menu) {
                    ...
                    $('#idmain_menu').val(main_menu.idmenus_2);
                    $('#labelfr').val(main_menu.labelfr);
                    $('#labeleng').val(main_menu.labeleng);
                    $('#menu_level_1').val(main_menu.menu_1).select2();
        ...
        function remove(idmain_menu) {
            swal({
                icon: 'warning',
                title: "{{$title}}",
                text: '@lang('confirm.menu_level_2_del_text')',
        ...
        function submitForm() {
            let text = '@lang('confirm.menu_level_2_save_text')';
            if ($('#idmain_menu').val() !== '') {
                text = '@lang('confirm.menu_level_2_edit_text')';
            }
        ...
        function in_out_form() {
            $('#title').text('@lang('label.new_menu_level_2')');
            $('#idmain_menu').val('');
            $('#fillform :input').val('');
            $('.select2').val('').select2();
