...
@section('content')
    <div class="box box-info" id="newForm" style="display: none;">
        <div class="box-header with-border">
            <h3 class="box-title text-bold" id="title">@lang('label.new_menu_level_4')</h3>
        ...
        <div class="box-body">
            <form action="{{ route('menu_level_4/store') }}" method="post" role="form" id="main_menuForm" class="needs-validation">
                {{ csrf_field() }}
                ...
                <div id="fillform">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 col-xs-12">
                            <div class="form-group has-error">
                                <label for="menu_level_3" class="col-md-3 col-xs-5 control-label">@lang('label.menu_level_3')<span class="text-red text-bold">*</span></label>
                                <div class="col-md-9 col-xs-7">
                                    <select class="form-control select2" id="menu_level_3" name="menu_level_3" required>
                                        <option value=""></option>
                                        @foreach($main_menus_3 as $main_menu_3)
                                            <option value="{{ $main_menu_3->idmenus_3 }}">@if($emp->lang == 'fr') {{ $main_menu_3->labelfr }} @else {{ $main_menu_3->labeleng }} @endif</option>
                                        @endforeach
                                    </select>
                                    <span class="help-block">@lang('placeholder.menu_level_3')</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        @if($emp->lang == 'fr')
                            <div class="col-md-5 col-xs-12">
                                <div class="form-group has-error">
                                    <label for="labelfr" class="col-md-3 col-xs-5 control-label">@lang('label.menu_level_4_fr')<span class="text-red text-bold">*</span></label>
                                    <div class="col-md-9 col-xs-7">
                                        <input type="text" class="form-control" name="labelfr" id="labelfr" required>
                                        <div class="help-block">@lang('placeholder.menu_level_4_fr')</div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-5 col-xs-12">
                                <div class="form-group has-error">
                                    <label for="labeleng" class="col-md-3 col-xs-5 control-label">@lang('label.menu_level_4_eng')<span class="text-red text-bold">*</span></label>
                                    <div class="col-md-9 col-xs-7">
                                        <input type="text" class="form-control" name="labeleng" id="labeleng" required>
                                        <div class="help-block">@lang('placeholder.menu_level_4_eng')</div>
                                    </div>
                                </div>
                            </div>
                        @else
        ...
        <div class="box-header with-border">
            ...
            <div class="box-tools pull-right">
                <button type="button" class="btn btn-success bg-green btn-sm pull-right" id="insertForm">
                    <i class="oin ion-plus"></i>&nbsp;@lang('label.new_menu_level_4')
                </button>
            </div>
        </div>
        <div class="box-body">
                    ...
                    <div class="table-responsive">
                        <table id="admin-data-table" class="table table-bordered table-striped table-hover table-responsive-xl">
                            ...
                            @foreach($main_menus_4 as $main_menu_4)
                                <tr>
                                    <td class="text-center">{{$main_menu_4->level}}</i></td>
                                    <td><span><i class="{{$main_menu_4->view_icon}}"></i></span>&nbsp;&nbsp; @if($emp->lang == 'fr') {{ $main_menu_4->labelfr }} @else {{ $main_menu_4->labeleng }} @endif</td>
                                    <td><span><i class="{{$main_menu_4->main_menu_3_view_icon}}"></i></span>&nbsp;&nbsp; {{$main_menu_4->main_menu_3}}</td>
                                    <td><span><i class="{{$main_menu_4->main_menu_2_view_icon}}"></i></span>&nbsp;&nbsp; {{$main_menu_4->main_menu_2}}</td>
                                    <td><span><i class="{{$main_menu_4->main_menu_1_view_icon}}"></i></span>&nbsp;&nbsp; {{$main_menu_4->main_menu_1}}</td>
                                    <td class="text-center">
                                        <button type="button" class="btn btn-info bg-aqua btn-sm fa fa-edit" onclick="edit('{{$main_menu_4->idmenus_4}}')"></button>
                                        <button type="button" class="btn bg-red btn-sm delete fa fa-trash" onclick="remove('{{$main_menu_4->idmenus_4}}')"></button>
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
                        <form action="{{route('menu_level_4/delete')}}" method="post" role="form" id="delForm" style="display: none">
                            {{ csrf_field() }}
    ...
    <script>
                ...
                $.ajax({
                    ...
                    success: function (menus_3) {
                        ...
                        $.each(menus_3, function (i, menu_3) {
                            option += "<option value=" + menu_3.idmenus_3 + ">@if($emp->lang == 'fr') " + menu_3.labelfr + " @else " + menu_3.labeleng + " @endif</option>";
                        });
                        $('#menu_level_3').html(option);
                    }
                });
            }
        });

        $('#menu_level_3').change(function () {
            if ($(this).val() !== '') {
                $.ajax({
        ...
        function remove(idmain_menu) {
            swal({
                icon: 'warning',
                title: "{{$title}}",
                text: '@lang('confirm.menu_level_4_del_text')',
        ...
        function submitForm() {
            let text = '@lang('confirm.menu_level_4_save_text')';
            if ($('#idmain_menu').val() !== '') {
                text = '@lang('confirm.menu_level_4_edit_text')';
            }
        ...
        function in_out_form() {
            $('#title').text('@lang('label.new_menu_level_4')');
            $('#idmain_menu').val('');
            $('#fillform :input').val('');
            $('.select2').val('').select2();
