...
@section('content')
    <div class="box">
        ...
        <div class="box-body">
            <form action="{{ url('other_check_out/store') }}" method="post" role="form" id="ocheoutForm" class="needs-validation">
                {{ csrf_field() }}
...
@section('script')
    <script>
        ...
        function submitForm() {
            ...
            if (!isNaN(tot)) {
                mySwal("{{ $title }}", '@lang('confirm.other_check_out_text')', '@lang('confirm.no')', '@lang('confirm.yes')', '#ocheoutForm');
            } else {
                myOSwal("{{ $title }}", '@lang('confirm.other_check_out_error_text')', 'error');
            }
        }
    </script>
@stop
