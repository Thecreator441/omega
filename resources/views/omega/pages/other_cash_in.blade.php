...
@section('content')
    <div class="box">
        ...
        <div class="box-body">
            <form action="{{ url('other_cash_in/store') }}" method="post" role="form" id="ocinForm" class="needs-validation">
                {{ csrf_field() }}
    ...
    <script>
        ...
        function submitForm() {
            if (parseInt(trimOver($('#diff').text(), null)) === 0) {
                mySwal("{{ $title }}", '@lang('confirm.ocin_text')', '@lang('confirm.no')', '@lang('confirm.yes')', '#ocinForm');
            } else {
                myOSwal("{{ $title }}", '@lang('confirm.ocinerror_text')', 'error');
            }
        }
    </script>
@stop
